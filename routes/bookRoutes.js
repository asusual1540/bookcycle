...
function more_like_this(id) {
  var related_books = [];
  Book.find({ _id: id }).then(book => {
    if (book) {
      Book.find({ language: language }).then(books => {
        related_books.push(...books);
      });
      Book.find({ category: category }).then(books => {
        related_books.push(...books);
      });
      Book.find({ author: author }).then(books => {
        related_books.push(...books);
      });
    } else {
...
router.post("/sell", isLoggedIn, upload, (req, res) => {
      ...
      (err, result) => {
        ...
        Author.findOne({ name: newAuthor.toLowerCase() })
          .then(author => {
            if (author) {
              return console.log(
                "Author by that name already exists in Database"
              );
            }
        ...
        Category.findOne({ name: newCategory.toLowerCase() })
          .then(category => {
            if (category) {
              return console.log(
                "Category by that name already exists in Database"
              );
            }
        ...
        var username =
          req.user.local.name || req.user.facebook.name || req.user.google.name;
  ...
  Book.findById(id).then(
    book => {
      ...
      Book.find({
        ...
        Book.find({
          $and: [{ author: book.author }, { name: book.name }]
        }).then(_books => {
          if (_books) {
            books = _books;
          }
